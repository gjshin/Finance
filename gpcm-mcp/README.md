...
**전제: 이 PC 에서 도는 Claude 여야 한다.**

서버는 이 PC 의 프로세스로 뜬다. 브라우저의 claude.ai 나 클라우드에서 도는 Claude Code
세션은 다른 컴퓨터라 이 PC 를 볼 수 없다 — 설치가 멀쩡해도 도구가 나타나지 않는다.
그 컴퓨터는 국내도 아니라 DART 도 막힌다. **클로드 데스크톱 앱에서 열어도 클라우드
세션일 수 있다**는 점이 특히 헷갈린다.

확인: Claude 에게 작업 폴더를 물어 `C:\...` 면 로컬, `/home/...` 면 클라우드다.

가장 확실한 방법은 터미널에서 직접 띄우는 것이다.
...
Claude Code 는 **연 폴더의 `.mcp.json`** 을 읽는다. 다른 폴더에서 열면 못 찾는다.

> 클로드 데스크톱 앱 자체에도 MCP 설정(`claude_desktop_config.json`)이 따로 있는데,
> 버전마다 메뉴 위치가 달라 여기서 단언하지 않는다. 위 방법으로 먼저 확인할 것.
